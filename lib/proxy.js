...
var debug = require('debug')('http-mitm-proxy');
...
var cachedServers=new NodeCache({stdTTL:200,checkperiod:100});
...
Proxy.prototype.listen = function(options,callback) {
  ...
  this.sslCertCacheDir = options.sslCertCacheDir;
...
Proxy.prototype._onHttpServerConnect = function(req, socket, head) {
  ...
  function makeConnection(port) {
    ...
    var target = net.connect(port, 'localhost');
      target.on('connect', ontargetconnect.bind(target));
      target.on('close', ontargetclose.bind(target));
      target.on('error', ontargeterror.bind(target));
      target.on('end', ontargetend.bind(target));
